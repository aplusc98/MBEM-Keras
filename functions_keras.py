...
def generate_labels_weight(train_label,val_label,n,n1,repeat,conf):
    ...
    for i in range(n):
        class_train[i] = int(float(train_label[i]))
    ...
    for i in range(n):
        ...
        for j in workers_this_example[i]:
            ...
            ### saving the responses of workers
            for x in range(10):
                if temp_rand[x] == 1:
                    temp[i,l] = x
                    l = l+1
    ...
    var = np.zeros(n)
    for i in range(n):
        var[i] = statistics.variance(temp[i,:])
    ...
    for i in range(n1):
        workers_val_label['softmax' + str(0) + '_label'][i][int(val_label[i])] = 1
    ...
    return resp, workers_train_label, workers_val_label, workers_this_example, var, temp
